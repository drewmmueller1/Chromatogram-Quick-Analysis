...
st.markdown("Upload your chromatogram data (CSV format with 'time' and 'intensity' columns) to visualize and auto-label peaks.")

# Sidebar for options
st.sidebar.header("📊 Plot Options")
peak_height = st.sidebar.slider("Minimum peak height for detection", 0.0, 1000.0, 10.0)
peak_distance = st.sidebar.slider("Minimum distance between peaks", 1, 50, 5)
...
if uploaded_file is not None:
    ...
    # Find peaks
    peaks, properties = find_peaks(intensity, height=peak_height, distance=peak_distance)
    ...
    if len(peaks) > 0:
        ...
        ax.plot(peak_times, peak_intensities, "ro", label=f'Detected Peaks ({len(peaks)})')
    ...
    ax.set_title("Chromatogram with Peak Detection")
    ax.legend()
    ...
    if len(peaks) > 0:
        ...
        st.warning("No peaks detected. Adjust the sliders in the sidebar.")
...
# Footer
st.markdown("---")
st.markdown("Built with ❤️ using Streamlit, Pandas, Matplotlib & SciPy")
